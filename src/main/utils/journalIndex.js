...
const pileSearchIndex = require('./journalSearchIndex');
const pileEmbeddings = require('./journalEmbeddings');
...
class JournalIndex {
  ...
  async load(journalPath) {
    ...
    const indexFilePath = path.join(this.journalPath, this.fileName);
    ...
    if (fs.existsSync(indexFilePath)) {
      ...
      const index = await this.walkAndGenerateIndex(journalPath);
      this.index = index;
      this.save();
    }

    pileSearchIndex.initialize(this.journalPath, this.index);
    console.log('📍 SEARCH INDEX LOADED');
    await pileEmbeddings.initialize(this.journalPath, this.index);
    console.log('📍 VECTOR INDEX LOADED');

    return this.index;
  }

  walkAndGenerateIndex = (journalPath) => {
    return walk(journalPath).then((files) => {
      files.forEach((filePath) => {
        const relativeFilePath = path.relative(journalPath, filePath);
  ...
  add(relativeFilePath) {
    const filePath = path.join(this.journalPath, relativeFilePath);
    const fileContent = fs.readFileSync(filePath, 'utf8');
  ...
  getThreadAsText(filePath) {
    try {
      let fullPath = path.join(this.journalPath, filePath);
...
module.exports = new JournalIndex();
